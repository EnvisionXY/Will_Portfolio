...
export function Navigation() {
  ...
  return (
    <>
      {/* Mobile Menu Backdrop - moved outside nav but with proper z-index */}
      {isMobileMenuOpen && (
        <div
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-sm border-b border-neutral-700/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden p-2 text-foreground hover:text-crank-orange-1 transition-colors duration-200 relative z-50"
          ...
          >
            <div className="pt-6 pb-8 space-y-2 border-t border-neutral-700/40 mt-4 bg-background relative">
              <button
                onClick={() => handleNavClick("work")}
                className="block w-full text-left font-body text-lg text-foreground hover:text-crank-orange-1 transition-colors duration-200 py-3 px-2"
              >
                Projekte
              </button>
